function cfg = NFS(base_path, video, vtype)
	%full path to the video's files
	if base_path(end) ~= '/' && base_path(end) ~= '\'
		base_path(end+1) = '/';
    end
    
	video_path = [base_path video '/' num2str(vtype) '/'];
	...
	try
		...
		frewind(f);
		ground_truth = textscan(f, '%f %f %f %f %f %f %f %f %f %s');  
	end
	ground_truth = cat(2, ground_truth{2:5});
    ground_truth(:, 3:4) = ground_truth(:, 3:4) - ground_truth(:, 1:2) + 1;
	fclose(f);
    
    if vtype == 30
        ground_truth = ground_truth(1:8:end, :);
    end

    cfg.startFrame = 1;
    cfg.endFrame = size(ground_truth, 1);
