function [y,mask] = vl_nnchnout(x,varargin)
...
if isempty(opts.mask)
  ...
  if isa(x,'gpuArray')
    opts.mask = scale * (gpuArray.rand(1, 1, size(x,3), size(x,4), 'single') >= opts.rate) ;
  else
    opts.mask = scale * (rand(1, 1, size(x,3), size(x,4), 'single') >= opts.rate) ;
  end
end
...
if ~backMode
  y = bsxfun(@times, opts.mask, x) ;
else
  y = bsxfun(@times, opts.mask, dzdy) ;
end
mask = opts.mask ;
