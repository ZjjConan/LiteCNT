...
function [newTargetPosition, bestScale, feature, response_final] = tracker_step_wrapper(net_x, s_x, s_z, scoreId, z_out, x_crops, pad_masks_x, targetPosition, window, p)
    % run a forward pass of the CNN
    net_x.eval([z_out, {'instance', x_crops}]);
    feature = net_x.vars(4).value;
    ...
    if ~isempty(p.gpus)
        responseMapsUP = gpuArray(responseMapsUP);
    end
    ...
    feature = feature(:,:,bestScale);
    response = responseMaps(:,:,bestScale);
