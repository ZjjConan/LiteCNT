function [bboxes, speed, searchArea, template, feature, response] = tracker_gt_TN(varargin)
    ...
    % additional batch normalization learning rate
    p.TNLearningRate = 0.02;
    ...
    net_z = BatchNorm2TempNorm(net_z, p.TNLearningRate*2);
    net_x = BatchNorm2TempNorm(net_x, p.TNLearningRate*2);
    
    net_x_index = find_bnorm_moment_index(net_x);
    net_z_index = find_bnorm_moment_index(net_z);
    
    if numel(net_x_index) ~= numel(net_z_index)
        net_x_index(end) = [];
    end
    ...
    % g_values
    [g_statics(1:numel(net_z_index)).value] = deal(net_z.params(net_z_index).value);
    
    net_z.eval({'exemplar', z_crop});
    
    % for TN layer update ----------------------
    [net_x.params(net_x_index).value] = deal(net_z.params(net_z_index).value);
    ...
    net_z = changeBNLr(net_z, p.TNLearningRate);
    net_x = changeBNLr(net_x, p.TNLearningRate);
    
    feature = cell(nImgs, 1);
    searchArea = cell(nImgs, 1);
    template = cell(nImgs, 1);
    response = cell(nImgs, 1);
    
    net_x.vars(4).precious = 1;
    net_z.vars(4).precious = 1;
    ...
    for i = p.startFrame:nImgs
        if i>p.startFrame
            ...
            [newTargetPosition, newScale, featr, resp] = tracker_step_wrapper(net_x, s_x, s_z, scoreId, z_out, x_crops, pad_masks_x, p.targetPosition, window, p);
            s_x = max(min_s_x, min(max_s_x, (1-p.scaleLR)*s_x + p.scaleLR*scaledInstance(newScale)));
            
            
            % for TN layer update ----------------------
%             [net_z.params(net_z_index).value] = deal(net_x.params(net_x_index).value);
            ...
            feature{i} = gather(featr);
            response{i} = gather(resp);
            searchArea{i} = gather(x_crops(:,:,:,newScale));
        ...
        template{i} = gather(net_z.vars(4).value);
...
%         if i > 1
%             p.targetPosition = p.ground_truth(i, [2,1]) + p.ground_truth(i, [4,3])/2;
%             p.targetSize =  p.ground_truth(i, [4,3])/2;
%         end
